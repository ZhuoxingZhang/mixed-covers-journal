...
public class exp0 {
	...
	public static void computeCovers(Parameter para,String memoryUnit) {
		...
		exp0.outputFDCoverStats(para.dataset.name, "non-redundant", FDs.size(), Utils.compFDAttrSymbNum(FDs), nonRedunCover.size(), count1, cost_nonredun, maxMemUse_nr, para.output_add);
		System.out.println("#########################\n");
		...
		exp0.outputFDCoverStats(para.dataset.name, "reduced", FDs.size(), Utils.compFDAttrSymbNum(FDs), redCover.size(), count2, cost_reduced, maxMemUse_reduced, para.output_add);
		System.out.println("#########################\n");
		...
		exp0.outputFDCoverStats(para.dataset.name, "canonical", FDs.size(), Utils.compFDAttrSymbNum(FDs), canonCover.size(), count3, cost_canonical, maxMemUse_canon, para.output_add);
		System.out.println("#########################\n");
		...
		exp0.outputFDCoverStats(para.dataset.name, "minimal", FDs.size(), Utils.compFDAttrSymbNum(FDs), minimalCover.size(), count4, cost_minimal, maxMemUse_mini, para.output_add);
		System.out.println("#########################\n");
		...
		exp0.outputFDCoverStats(para.dataset.name, "reduced minimal", FDs.size(), Utils.compFDAttrSymbNum(FDs), red_minimal_Cover.size(), count5, cost_red_min, maxMemUse_red_mini, para.output_add);
		System.out.println("#########################\n");
...
//		System.out.println("simplified bool expression : "+boolEx);
		...
		exp0.outputFDCoverStats(para.dataset.name, "optimal", FDs.size(), Utils.compFDAttrSymbNum(FDs), optimalCover.size(), count6, cost_opt, maxMemUse_opt, para.output_add);
		System.out.println("#########################\n");
	}
	...
	public static void outputFDCoverStats(String datasetName, String coverType,int originFDNum, int originFDAttrSymNum, int FDNum,int attrSymbNum,long costInMs,double usedMemory,String output) {
		//output some properties of FD covers
		try {
			FileWriter fr = new FileWriter(output,true);
			BufferedWriter br = new BufferedWriter(fr);
			br.write(datasetName + "," + coverType + "," + originFDNum + "," + originFDAttrSymNum + "," + FDNum + "," + attrSymbNum+","+costInMs+","+usedMemory+"\n");
			br.close();
			fr.close();
		}catch(IOException e) {
			e.printStackTrace();
	...
	public static void main(String[] args) throws IOException {
		...
		Utils.getParameterListV1(null).forEach(para -> computeCovers(para, "MiB"));
